SageCoin Core version *0.15.1* is now available from:

  <https://download.sagecoin.org/sagecoin-0.15.1/>
...
A number of changes to the way SageCoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 magic per kB (0.00001 SGC). This is to prepare for dropping the minimum transaction fee to 0.1 magic per kB (0.0001 SGC) in the next release.
